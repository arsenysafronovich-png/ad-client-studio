...
- Не используй литературщину, псевдо-живые образы и драматичные болевые клише: "утренние ступеньки", "спуск злится", "колено злится", "вечерний круг", "тянет под надколенником", "прострел в пояснице", "снова отзывается", "схватило", "скрутило", "прихватило", "пробежка по набережной" как заголовок, "тело просит", "зона держит", "шея не отпускает". Это звучит как модель, которая старается быть писателем.
- Не используй синтаксические обрубки и кривую бытовую сцену: "Встаете из кровати или со стула, и...", "Наклониться за пакетом, снова...", "и прострел", "снова отзывается". Это не живой разговор, а сломанный русский. Нормально: "Когда встаете с кровати или со стула, появляется резкая боль в пояснице" / "Наклоняетесь за пакетом, и боль снова появляется".
...
function assessCopyQuality(text, body) {
  ...
  const crookedPhrases = [
    ...
    "и прострел",
    "снова отзывается",
    "встаете из кровати или со стула, и",
    "наклониться за пакетом, снова",
    "схватило",
    "скрутило",
    "прихватило"
  ];
  ...
  if (/набережн|утренн(ие|яя)\s+ступеньк|под\s+надколенник|вечерн(ий|ем)\s+круг|прострел|отзывается|схватило|скрутило|прихватило/i.test(checkedText)) {
    issues.push("слишком писательский образ вместо нормальной речи клиента");
  }

  if (/вста(е|ё)те\s+из\s+кровати|вста(е|ё)те[^.!?\n]{0,50},\s*и\s+(резкая\s+боль|прострел|болит)|наклониться\s+за\s+пакет|,\s*снова\s+(появляется\s+боль|отзывается)/i.test(checkedText)) {
    issues.push("кривая бытовая сцена: обрубленная грамматика вместо живой фразы");
  }
  ...
  if (firstTextLine && /набережн|ступеньк|утренн|вечерн(ий|ем)\s+круг|под\s+надколенник|прострел|отзывается|схватило|скрутило|прихватило/i.test(firstTextLine)) {
...
function sanitizeCopyOutput(text) {
  return String(text || "")
    ...
    .replace(/встаете\s+из\s+кровати\s+или\s+со\s+стула,\s+и/gi, "Когда встаете с кровати или со стула,")
    .replace(/наклониться\s+за\s+пакетом,\s+снова/gi, "Наклоняетесь за пакетом, и снова")
    .replace(/,\s+и\s+прострел\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице")
    .replace(/и\s+прострел\s+в\s+пояснице/gi, "появляется резкая боль в пояснице")
    .replace(/,\s+прострел\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице")
    .replace(/прострел(ивает|ило|ы|а)?/gi, "резкая боль")
    .replace(/,\s+резкая\s+боль\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице")
    .replace(/снова\s+отзывается/gi, "снова появляется боль")
    .replace(/отзывается/gi, "появляется боль")
    .replace(/схватило/gi, "резко заболело")
    .replace(/скрутило/gi, "стало больно двигаться")
    .replace(/прихватило/gi, "резко заболело")
...
- запрещены фразы в стиле: "утренние ступеньки", "спуск злится", "тянет под надколенником", "вечерний круг", "прострел", "и прострел", "отзывается", "снова отзывается", "схватило", "скрутило", "прихватило", "встаете из кровати", "наклониться за пакетом";
