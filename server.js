...
function sanitizeCopyOutput(text) {
  return String(text || "")
    ...
    .replace(/\bпрострел(ивает|ило|ы|а)?\b/gi, "резкая боль")
    .replace(/снова\s+отзывается/gi, "снова появляется боль")
    .replace(/\bотзывается\b/gi, "появляется боль")
    .replace(/\bсхватило\b/gi, "резко заболело")
    .replace(/\bскрутило\b/gi, "стало больно двигаться")
    .replace(/\bприхватило\b/gi, "резко заболело")
...
- запрещены фразы в стиле: "утренние ступеньки", "спуск злится", "тянет под надколенником", "вечерний круг", "прострел", "отзывается", "схватило", "скрутило", "прихватило";
