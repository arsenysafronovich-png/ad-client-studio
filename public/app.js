...
function crookedPhrasePass(text, client) {
  const replacements = [
    ...
    [/\bпрострел(ивает|ило|ы|а)?\b/gi, "резкая боль"],
    [/снова\s+отзывается/gi, "снова появляется боль"],
    [/\bотзывается\b/gi, "появляется боль"],
    [/\bсхватило\b/gi, "резко заболело"],
    [/\bскрутило\b/gi, "стало больно двигаться"],
    [/\bприхватило\b/gi, "резко заболело"]
  ];
