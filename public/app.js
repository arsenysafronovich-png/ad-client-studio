...
function crookedPhrasePass(text, client) {
  const replacements = [
    ...
    [/встаете\s+из\s+кровати\s+или\s+со\s+стула,\s+и/gi, "Когда встаете с кровати или со стула,"],
    [/наклониться\s+за\s+пакетом,\s+снова/gi, "Наклоняетесь за пакетом, и снова"],
    [/,\s+и\s+прострел\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице"],
    [/и\s+прострел\s+в\s+пояснице/gi, "появляется резкая боль в пояснице"],
    [/,\s+прострел\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице"],
    [/прострел(ивает|ило|ы|а)?/gi, "резкая боль"],
    [/,\s+резкая\s+боль\s+в\s+пояснице/gi, ", появляется резкая боль в пояснице"],
    [/снова\s+отзывается/gi, "снова появляется боль"],
    [/отзывается/gi, "появляется боль"],
    [/схватило/gi, "резко заболело"],
    [/скрутило/gi, "стало больно двигаться"],
    [/прихватило/gi, "резко заболело"]
  ];
...
function stopCranePass(text, client) {
  ...
  if (/утренн(ие|яя)\s+ступеньк|спуск.*злит|колено.*злит|вечерн(ий|ем)\s+круг|тянет\s+под\s+надколенник|набережн|прострел|отзывается|схватило|скрутило|прихватило/i.test(text)) warnings.push("литературщина вместо нормальной речи клиента");
  if (/вста(е|ё)те\s+из\s+кровати|вста(е|ё)те[^.!?\n]{0,50},\s*и\s+(резкая\s+боль|прострел|болит)|наклониться\s+за\s+пакет|,\s*снова\s+(появляется\s+боль|отзывается)/i.test(text)) warnings.push("кривая бытовая сцена и обрубленная грамматика");
